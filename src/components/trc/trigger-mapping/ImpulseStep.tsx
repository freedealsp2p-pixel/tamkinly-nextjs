...
interface ImpulseStepProps {
  locale: Locale;
  currentImpulse: string;
  currentActualResponse: string;
  onUpdate: (updates: { impulse?: string; actualResponse?: string }) => void;
  onNext: () => void;
  onSkip: () => void;
}

export default function ImpulseStep({
  locale,
  currentImpulse,
  ...
  const [impulse, setImpulse] = useState(currentImpulse);
  const [actualResponse, setActualResponse] = useState(currentActualResponse);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
      ...
      >
        ...
        <p className="text-slate-500 mb-8">{t.impulseStepSubtitle}</p>
        ...
        <div className="mb-6">
          ...
          <textarea
            value={impulse}
            onChange={(e) => { setImpulse(e.target.value); onUpdate({ impulse: e.target.value }); }}
            placeholder={t.impulsePlaceholder}
            rows={3}
        ...
        <div className="mb-10">
          <label className="block text-sm font-medium text-[#0F1C2E] mb-2">
            {t.actualResponseLabel}
          </label>
          <textarea
            value={actualResponse}
            onChange={(e) => { setActualResponse(e.target.value); onUpdate({ actualResponse: e.target.value }); }}
            placeholder={t.actualResponsePlaceholder}
            rows={3}
            className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white text-[#0F1C2E] placeholder:text-slate-400 focus:border-[#1F6F78] focus:ring-1 focus:ring-[#1F6F78] transition-colors resize-none"
          />
        </div>
